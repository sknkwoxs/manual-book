...
| 구분 | 파일 수 | 설명 |
|------|---------|------|
| 페이지 템플릿 (PHP) | 12개 | 페이지 컨트롤러 |
| Twig 템플릿 | 16개 | 프론트엔드 뷰 |
| PHP 모듈 (inc/) | 5개 | 기능 모듈 |
| JavaScript | 15개+ | 인터랙션/애니메이션 |
...
| 13 | `404.php` | `/themes/sknk_80/` | 404 에러 페이지 |
...
| 11 | `404.twig` | `/twigs/` | 404 에러 뷰 |

### 3.2 Partials (공통 컴포넌트)
...
flowchart TB
    subgraph CORE["테마 코어"]
        FUNC["functions.php<br/>(테마 초기화, inc 로드)"]
    end
    
    subgraph INC["inc/ 모듈"]
        REG["register.php<br/>(스크립트 등록)"]
        CPT["event-cpt.php<br/>(CPT 등록)"]
        COMM["event-comments.php<br/>(댓글 기능)"]
        AJAX["ajax.php<br/>(AJAX 처리)"]
    end
    
    subgraph ASSETS["프론트엔드 자원"]
        JS["src/js/*<br/>(JS 모듈들)"]
    end
    
    subgraph ADMIN["관리자 기능"]
        ADMIN_COMM["event-comments-admin.php<br/>(댓글 관리)"]
    end
