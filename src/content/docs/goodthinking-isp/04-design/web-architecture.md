...
| 구분 | 기술 | 비고 |
|------|------|------|
| Framework | NestJS (Node.js) | 모놀리식 모듈 구조 (Tradeoff 결정) |
| API | RESTful API | OpenAPI 문서화 |
| Authentication | JWT + OAuth2 | |
| ORM | TypeORM / Prisma | PostgreSQL 네이티브 지원 |

### Database

| 구분 | 기술 | 비고 |
|------|------|------|
| Primary DB | PostgreSQL (AWS RDS) | MSSQL에서 전환 — 라이선스 비용 제거, 오픈소스 생태계 활용 |
| Cache | Redis | 세션, 캐시 |
...
| 구분 | 기술 | 비고 |
|------|------|------|
| Cloud | AWS (서울 리전) | 전면 클라우드 전환 (Tradeoff 결정) |
| Container | Docker + ECS Fargate | K8s 대비 운영 복잡도 낮음 (Tradeoff 결정) |
| Orchestration | ECS (Fargate) | 15인 규모에 K8s 과잉 |
...
|------|--------|----------|
| YYYY-MM-DD | - | 초안 작성 |
| 2026-04-20 | 김명직 | DB를 PostgreSQL로 확정 (MSSQL 전환), 기술 스택 확정, AWS 인프라 월간 비용 산정 추가 (Prod+Staging+Bastion+NAT, 전환기간 이중운영 포함) |
