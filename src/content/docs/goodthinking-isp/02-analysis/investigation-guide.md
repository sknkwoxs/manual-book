...
| 자료 | 요청 대상 | 용도 | 상태 |
|------|----------|------|:----:|
| **DB 스키마 덤프 (DDL)** | 외부 개발자 | 테이블 구조, 관계 파악 | 미착수 (DB 정의서로 대체 분석 완료) |
| **Stored Procedure 목록/코드** | 외부 개발자 | 핵심 비즈니스 로직 파악 | 미착수 (정의서 기반 20 SP 목록 확보) |
| **DB 접속 정보** | 외부 개발자/IT | 직접 데이터 분석 | 미착수 (VPN 접근 선행 필요) |
| **주요 화면 스크린샷** | 정기구독팀 (사용자) | 기능 매핑 | 미착수 |

### 권장 자료 (Should Have)

| 자료 | 요청 대상 | 용도 | 상태 |
|------|----------|------|:----:|
| XPlatform 소스코드 | 외부 개발자 | 화면별 로직 분석 | 미착수 |
| 기존 설계 문서 | 외부 개발자 | 시스템 이해 단축 | 미착수 (없을 가능성 높음) |
| 운영 매뉴얼 | 정기구독팀/영업추진팀 | 업무 흐름 파악 | 미착수 |
...
| 자료 | 요청 대상 | 용도 | 상태 |
|------|----------|------|:----:|
| 과거 개발 히스토리 | 외부 개발자 | 변경 이력 파악 | 미착수 |
| 장애 이력 | 경영지원팀 | 취약점 파악 | 미착수 |
| 사용자 요청 이력 | 정기구독팀 | 개선 포인트 파악 | 미착수 |
...
| 3 | 사용자에게 "이렇게 동작하나요?" 검증 | [추정] 미수행 (인터뷰 일정 미확정) | 미착수 |
...
- [업무 분석 (워크쉐도잉)](./business-analysis) - 업무 흐름 분석 결과 (완료: 문서 기반 작성 — 4팀 플로우, 수작업 12건, 병목 7건. 인터뷰 후 정량 확정)
