...
| 산출물 | 설명 | 상태 |
|--------|------|:----:|
| 현행 업무흐름도 | 현재 업무 프로세스 시각화 | 미착수 |
| AS-IS ERD (역공학) | 현행 DB 구조 분석 결과 | 미착수 |
| 시스템 구조적 문제 진단서 | 현행 시스템의 문제점 정리 | 미착수 |
| 이해관계자 인터뷰 결과 | 팀별 인터뷰 내용 및 요구사항 | 미착수 |
| 요구사항 정의서 (초안) | 현업 요구사항 수집 결과 | 미착수 |
...
```mermaid
graph TD
    A["인터뷰 대상 선정"] --> B["질문지 준비"] --> C["팀별 인터뷰<br/>(실무자 화면 시연 병행)"]
    C --> D["업무 절차·Pain Point 기록"] --> E["결과 분석"] --> F["요구사항 종합"]
```

### 3. 요구사항 수집
```mermaid
graph LR
    A["화면 시연 인터뷰"] --> D["요구사항 목록화"]
    C["기존 자료 분석"] --> D
```
