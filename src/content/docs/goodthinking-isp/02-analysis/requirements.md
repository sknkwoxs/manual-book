...
:::note[분석 기준]
아래 기능 요구사항은 DB 정의서 분석(C/S 75t, 홈페이지 63t, CMS 13t) + 업무 플로우 분석(4개 팀) 결과를 기반으로 작성되었습니다.
- **현행 지원**: 현재 시스템에서 해당 기능이 어떻게 지원되는지 DB/로직 근거 기반 기술
- **우선순위**: 인터뷰 미수행 상태이므로 DB 분석 및 병목 분석 기반 잠정 배정. 인터뷰 후 확정 필요.
- 🔶 = 추정 (인터뷰 후 검증 필요)
...
> RFP [요구기능정의서](/goodthinking-isp/04-implementation/rfp-preparation/)의 12모듈 체계와 동일 구조
