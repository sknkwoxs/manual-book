...
> [업무 분석](/goodthinking-isp/02-analysis/business-analysis/)에서 식별된 수작업 12건을 시스템 아키텍처 구성 요소에 매핑한 결과.
...
│                                                                         │
│  [외부 → On-Prem]                                                       │
│  ❶ Playauto ──(수동 Excel)──→ CS System        (외부몰 주문 다운로드)    │
│  ❷ Excel 파일들 ──(복사/붙여넣기)──→ 통합 Excel   (데이터 취합)          │
│  ❺ 나이스페이 PG ──(수동 확인)──→ CS System      (결제-구독 매칭)        │
│  ❻ 지로 입금 파일 ──(수동 대조)──→ CS System      (지로 입금 확인)        │
│                                                                         │
│  [On-Prem 내부]                                                          │
│  ❸ Excel ──(건별 수동 입력)──→ CS System          (CS 프로그램 수동 입력) │
│  ⓫ CS System ──(육안 대조)──→ 정합성 확인         (데이터 검증)          │
│  ❽ CS System ──(수동 업로드)──→ 우체국/CJ 시스템   (배송 라벨 출력)      │
│                                                                         │
│  [On-Prem → AWS] ⚠️ 핵심 단절 구간                                      │
│  ❹ CS(PT_Finance) ──(수동)──→ CMS(ptcms_member) (CMS 권한 수동 부여)    │
│  ❼ 홈페이지(PTM_Orders) ──(수동 재입력)──→ CS(PT_Customer)              │
│  ⓬ CS(sp_SubscribeCancel) ──(수동)──→ CMS(ptcms_member) (구독 만료)    │
│                                                                         │
│  [On-Prem → 외부 SaaS]                                                   │
│  ❾ CS System ──(Excel 취합)──→ 경영지원팀        (매출 데이터 취합)      │
│  ❿ CS 매출 데이터 ──(수동 입력)──→ 이카운트 ERP   (ERP 이중 입력)        │
│                                                                         │
│  병목 심각도: ❹❼⓬ On-Prem↔AWS 단절 = 🔴 Critical                       │
│              ❶❷❸❺❻ 외부↔On-Prem 수동 = 🔴 Critical                     │
│              ❾❿ On-Prem↔ERP 이중 작업 = 🔴 Critical                     │
│              ❽⓫ 내부 수동 = 🟠 Major / 🟢 Minor                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```
 ...
 └──────┬───────────┘
        ...
        ▼                      ▼
 ┌──────────────────┐  ┌──────────────────┐
 │PTM_Payment_Infos │  │PTM_ShippingInfos │
 ├──────────────────┤  ├──────────────────┤
 │ FK order_id      │  │ FK order_id      │
 │    pay_method    │  │    shipping_co   │
 │    pg_tid        │  │    tracking_no   │
 └──────────────────┘  └──────────────────┘

 ┌──────────────────┐         ┌──────────────────┐
 │ PTM_Regulars     │────────►│PTM_Regular_Orders│
 ├──────────────────┤         ├──────────────────┤
 │ PK regular_id    │         │ FK regular_id    │
 │    member_id     │         │    order_date    │
 │    period        │         │    status        │
 └──────────────────┘         └──────────────────┘

 * PTM_Orders가 주문 허브, PTM_Products가 상품 허브
 * 정기구독은 PTM_Regulars → PTM_Regular_Orders로 별도 관리
 * 선물은 PTM_GiftOrders → PTM_GiftOrder_Items 별도 체계
```

#### 3개 DB 간 연관 관계 (Cross-DB)

```
 ┌─────────────────────────────────────────────────────────────────────┐
 │              3개 DB 간 연관 관계 (⚠️ 모두 수동 이관)                  │
 ├─────────────────────────────────────────────────────────────────────┤
 │                                                                     │
 │  C/S 고객관리 DB              홈페이지 DB                             │
 │  ┌─────────────┐             ┌─────────────┐                        │
 │  │PT_Customer  │◄─·수동·───►│(PTM_Members)│  ⚠️ 고객 동기화 없음    │
 │  │Customer_ID  │  (Excel)    │ member_id   │                        │
 │  └──────┬──────┘             └──────┬──────┘                        │
 │         │                           │                               │
 │  ┌──────┴──────┐             ┌──────┴──────┐                        │
 │  │PT_Subscribe │◄─·수동·───►│PTM_Regulars │  ⚠️ 구독 동기화 없음    │
 │  │(구독 접수)   │  (Excel)    │(정기구독)    │                        │
 │  └──────┬──────┘             └──────┬──────┘                        │
 │         │                           │                               │
 │  ┌──────┴──────┐             ┌──────┴──────┐                        │
 │  │PT_Finance   │◄─·수동·───►│PTM_Payment  │  ⚠️ 결제 동기화 없음    │
 │  │(입금/환불)   │  (수작업)   │_Infos       │                        │
 │  └─────────────┘             └─────────────┘                        │
 │                                                                     │
 │  C/S 고객관리 DB              CMS DB                                  │
 │  ┌─────────────┐             ┌─────────────┐                        │
 │  │PT_Finance   │──·수동·───►│ptcms_member │  ⚠️ 결제→권한 수동      │
 │  │(결제 확인)   │             │(열람 권한)   │  (수작업 ❹)            │
 │  └─────────────┘             └─────────────┘                        │
 │                                                                     │
 │  ┌─────────────┐             ┌─────────────┐                        │
 │  │PT_Book      │──·수동·───►│ptcms_books  │  ⚠️ 도서 정보 수동      │
 │  │(도서 마스터)  │             │(CMS 도서)    │  (동기화 없음)          │
 │  └─────────────┘             └─────────────┘                        │
 │                                                                     │
 │  핵심 문제:                                                          │
 │  • 고객 1명 = 최대 3곳 분산 (PT_Customer + PTM_Members + ptcms_member)│
 │  • 동일 도서 = 2곳 (PT_Book + ptcms_books)                          │
 │  • 주문/결제 = 2곳 (PT_Finance + PTM_Payment_Infos)                  │
 │  • ⚠️ 통합 키(Unified Key)가 없음 — 이관 시 매핑 테이블 필수          │
 └─────────────────────────────────────────────────────────────────────┘
```
...
> 49개 비즈니스 로직(C/S 20 SP + 14 Function + 15 Trigger)과 홈페이지 7 SP를 업무 도메인별로 재분류. 웹 전환 시 각 도메인 단위로 API/서비스 레이어를 설계해야 함.

| 도메인 | SP | Function | Trigger | 홈페이지 SP | 합계 | 전환 복잡도 |
|--------|:--:|:--------:|:-------:|:-----------:|:----:|:----------:|
| **구독 관리** | 2 (Create, Cancel) | 1 (CalcPeriod) | 5 (Giro×3, Count×2) | 0 | **8** | 높음 |
| **결제/정산** | 3 (Finance×2, Nicepay) | 2 (DeferIncome, FinanceSum) | 2 (DeferIncome, DepositSync) | 2 (OrderItemReg, PaymentLog) | **9** | 높음 |
| **배송/물류** | 2 (SendBook, SendDM) | 3 (dailySend×3) | 1 (Receiver_SendHistory) | 1 (StockReturn) | **7** | 중간 |
| **지로** | 6 (CRUD+Send+Recv+Cancel) | 0 | 0 | 0 | **6** | 중간 |
| **도서/재고** | 1 (StockUpdate) | 2 (sendBook×2) | 2 (Book_Price, Company_Stock×1) | 0 | **5** | 낮음 |
| **고객 관리** | 2 (CustomerMerge, ReceiverUpdate) | 2 (GetNumeric, GetCustomerByTel) | 1 (Customer_Tel_Index) | 0 | **5** | 중간 |
| **선물** | 0 | 0 | 3 (GiftSend_Stock×3) | 0 | **3** | 낮음 |
| **쿠폰** | 2 (CouponCreate, CouponUse) | 0 | 0 | 0 | **2** | 낮음 |
| **시스템/모니터링** | 1 (DataMonitoring) | 2 (CheckDuplicate, FormatDate) | 0 | 2 (CartDelete, MMS+알림톡) | **5** | 낮음 |
| **거래처** | 0 | 1 (GetReceiverCount) | 2 (Company_Stock_Group×2) | 0 | **3** | 낮음 |
| **기타** | 1 (sendBookNo) | 1 (sendBookNo) | 0 | 2 (VbankAlert, BIZ_AT) | **3** | 낮음 |
| **합계** | **20** | **14** | **15** | **7** | **56** | - |

:::note[전환 전략]
- **높음 복잡도** (구독, 결제): 기존 SP 로직을 NestJS 서비스 레이어로 1:1 전환 후 트랜잭션 보장 테스트 필수
- **중간 복잡도** (배송, 지로, 고객): 외부 시스템 연동이 포함되어 API 설계 시 에러 핸들링 중요
- **낮음 복잡도** (도서, 선물, 쿠폰 등): 단순 CRUD 위주, 빠른 전환 가능
:::

### 핵심 테이블 연관 관계 매트릭스

> C/S 고객관리 DB의 핵심 테이블 간 FK/참조 관계. 웹 전환 시 이 관계를 TO-BE ERD에 보존해야 함.

| 테이블 | Customer | Subscribe | Receiver | Finance | Book | Company | Giro | GiftSend |
|--------|:--------:|:---------:|:--------:|:-------:|:----:|:-------:|:----:|:--------:|
| **PT_Customer** | - | 1:N | 1:N:N | 1:N | - | - | 1:N | 1:N |
| **PT_Subscribe** | FK | - | 1:N | - | - | - | 1:1 | - |
| **PT_Receiver** | FK | FK | - | - | FK(Book) | - | - | - |
| **PT_Finance** | FK | - | - | - | - | - | - | - |
| **PT_Book** | - | - | (via Recv) | - | - | - | - | - |
| **PT_Company** | - | - | - | - | - | - | - | - |
| **PT_Giro** | FK | FK | - | - | - | - | - | - |
...
> **읽는 법**: 행(Row) 테이블이 열(Column) 테이블을 FK로 참조. 예: PT_Subscribe 행 × Customer 열 = FK (구독이 고객을 참조)
> **1:N**: Customer → Subscribe는 1명의 고객이 여러 구독 가능
> **1:N:N**: Customer → Subscribe → Receiver는 3단계 복합키 체계
...
| 지표 | 수치 | 비고 |
|------|------|------|
| **총 테이블** | 151개 | C/S 75 + CMS 13 + 홈페이지 63 |
| **비즈니스 로직** | 56개 | SP 27 + Function 14 + Trigger 15 |
| **수작업 구간** | 12건 | [업무 분석](/goodthinking-isp/02-analysis/business-analysis/) 참조 |
| **병목 구간** | 7건 | Critical 3 + Major 3 + Minor 1 |
| **일일 수작업 시간** | 6.5~9시간 | 전 직원 합산 🔶 추정 |
| **고객 정보 분산** | 3곳 | PT_Customer + PTM_Members + ptcms_member |
| **DB 간 API 연동** | 0건 | C/S↔홈페이지↔CMS 모두 수동 |
| **외부 시스템 연동** | 7건 | 나이스페이, Playauto, CJ, CTI, 이카운트, 카카오 알림톡, 지로 |

### 시스템 성숙도 평가

| 영역 | 평가 | 등급 | 근거 |
|------|------|:----:|------|
| **아키텍처** | C/S 로컬 설치형, 이원화된 인프라 | 🔴 D | XPlatform 단종 위험, IE 의존 |
| **데이터 통합** | 3개 DB 물리적 분리, 수동 이관 | 🔴 D | 통합 키 부재, 데이터 정합성 미보장 |
| **자동화** | 수작업 비율 40~55% (추정) | 🔴 D | 12건 수작업, 7건 병목 |
| **보안** | PPTP VPN, 방화벽만 | 🟠 C | VPN 취약, RBAC 미구현, 감사 로그 부재 |
| **확장성** | 로컬 PC 의존, 동시접속 제한 | 🟠 C | 원격 근무 제약, 단일 장애점 |
| **문서화** | 2015~2023 문서 존재, 최신화 부족 | 🟠 C | DB 정의서 있으나 API 문서 부재 |
| **외부 연동** | PG/CTI 연동 존재 | 🟡 B | 나이스페이 PG 정상 작동, CTI 유지보수 |

### 핵심 진단 결론

1. **시스템 이원화가 모든 문제의 근본 원인**: On-Prem(C/S) ↔ AWS(웹) 간 API 연동이 전무하여 12건의 수작업과 7건의 병목이 발생. 단일 통합 DB로의 전환이 필수.

2. **비즈니스 로직의 DB 집중**: 56개 비즈니스 로직 중 대부분이 MSSQL SP/Function/Trigger에 구현. 웹 전환 시 이 로직들을 서비스 레이어(NestJS)로 체계적 이관 필요. 특히 구독(8건)과 결제/정산(9건) 도메인의 복잡도가 가장 높음.

3. **3곳 분산된 고객 데이터**: 동일 고객이 C/S, 홈페이지, CMS에 각각 별도 등록되어 통합 키가 없음. 마이그레이션 시 고객 매핑 테이블 설계가 최우선 과제.

4. **자동화 기대효과**: 수작업 75~80% 절감 가능 (6.5~9시간 → 1~2시간/일). 단, 현재 수치는 문서 기반 추정이므로 인터뷰/워크 쉐도잉을 통한 실측 확정 필요.
