...
| | Ecount-ERP | SaaS | 거래처/재고수불부 엑셀 연동 | 엑셀 양방향 |
...
| **외부 연동** | 나이스페이(PG/CTI), 금융결제원(지로), Ecount-ERP, Playauto |
...
| 화면ID | 화면명 | 핵심 기능 |
|--------|--------|-----------|
| BS3110 | 사원관리 | CRUD, 관리등급/부서/메뉴·버튼 권한, **Ecount 사원번호 매핑**, 논리삭제 |
...
| BS3117 | 거래처관리 | CRUD + **가상계좌 발급/회수** + **4종 결제** + 거래중지/재거래 + **Ecount 엑셀 양방향** |
...
| **Ecount-ERP** | 엑셀 양방향 (거래처/재고/영업) | BS3117, SD3515, SD3530 | 수시 (수동) |
...
| **수동 데이터 연동** | Playauto → CS 수동 Excel, Ecount 엑셀 양방향 | 처리 지연, 오류 가능성 | High |
...
| **엑셀 의존도** | Ecount 연동·Playauto 주문·영업리포트 모두 Excel 경유 | 데이터 정합성 위험 | High |
...
| Ecount 엑셀 양방향 | ERP API 직접 연동 | 실시간 재고/매출 동기화 |
...
- [x] 사용자 매뉴얼 V2.4 분석 완료 (74개 화면)
- [x] 관리자 매뉴얼 V1.1 분석 완료 (12개 화면)
- [x] CS 프로그램 화면 목록 CSV 작성 완료 (86개)
- [x] 인프라 상세 정보 확인 (IP/OS/WAS/DB)
- [x] 외부 연동 현황 파악 (7종)
- [x] 로그 관리 구조 파악 (나이스페이 + 웹연동)
- [x] 추정 ERD 작성 (매뉴얼 기반)
