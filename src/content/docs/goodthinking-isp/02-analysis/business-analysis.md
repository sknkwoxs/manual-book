---
title: 2.4. 업무 분석
description: 각 팀 화면 시연 인터뷰를 통한 현행 업무 분석
---

# 업무 분석

각 팀 화면 시연 인터뷰(실무자가 실제 시스템 화면을 시연하며 업무 절차를 설명)를 통해 개선이 필요한 병목 및 중복 업무를 식별합니다.
...
| 화면 시연 인터뷰 | 실무자 5명 + 외부 개발자 1명 대상, 실제 시스템 화면 시연을 통한 업무 절차 청취 및 Pain Point 식별 | 완료 (정기구독팀/편집팀/경영지원팀/외주콜센터/영업추진팀 5건 + CEO 1건) |

### 화면 시연 인터뷰 수행 결과

실무자가 실제 시스템 화면을 시연하며 업무 절차와 데이터 흐름을 설명, 동시에 Pain Point를 청취

| 대상 팀     | 대상자         | 기간    | 방법           | 상태 |
|------------|---------------|--------|---------------|:----:|
| 정기구독팀 | 정황규, 어은진 | 1\~2일 | 인터뷰 + 화면 시연 | 완료 |
| 영업추진팀 | 이성수, 권지은 | 1일 | 인터뷰 + 화면 시연 | 완료 |
| 경영지원팀 | 송윤경 | 0.5일 | 인터뷰 + 화면 시연 | 완료 |

> **[참고]** 분석 기반
>
> 아래 업무 흐름도 및 수작업 분석은 업무플로우 PDF 4건, 편집팀 업무플로우 PDF, CMS 개선사항 PDF, DB 정의서 분석 결과를 기반으로 작성되었으며, 화면 시연 인터뷰를 통해 검증·보정되었습니다.

---

## 현행 업무 흐름도

> 조직도 기반 5개 팀의 업무 플로우를 문서 분석(업무플로우 PDF #19-22 + 편집팀 업무플로우 PDF) 및 DB 역공학을 통해 정리. 정량 데이터(소요 시간, 건수)는 [추정] 추정이며, 화면 시연 인터뷰를 통해 검증 완료. 일부 항목은 실측 보정 반영.

### 1. 정기구독팀 (정황규, 어은진) — 5단계

```mermaid
flowchart LR
    A["① 구독 접수<br/>(전화/웹/외부몰<br/>CS/콜센터)"] -->|결과| B["② 결제 확인<br/>(나이스페이<br/>PG/지로 확인)"]
    B -->|확인| C["③ 발송 처리<br/>(CS 프로그램<br/>발송 데이터 생성)"]
    C -->|배송| D["④ 배송 관리<br/>(우체국/<br/>CJ대한통운)"]
    D -->|문의| E["⑤ CS 처리<br/>(구독변경/취소<br/>환불/문의 대응)"]
```
...
| 채널 | 담당 | 사용 시스템 | 수작업 여부 |
|------|------|-----------|:----------:|
| 단행본 유통 | 이성수(영업),<br> 권지은(출고) | SCM(예스24/알라딘/교보), 고려출판물류 시스템, 위하고 | **수동**<br>(이중 입력) |
| 총판 관리<br>(44군데) | 이성수(협의),<br> 권지은(보조) | 전화, 엑셀, 위하고 | **수동**<br>(전화+엑셀) |
| 다량·특판<br>(B2B) | 권지은(관리),<br> 이성수(계약) | 메일/전화, 개인 엑셀, 위하고 | **수동**<br>(삼중 입력) |
...
```mermaid
flowchart LR
    A["① 입금 확인·배분<br/>(인사이트 뱅크<br/>→ 구글 드라이브)"] -->|배분| B["② 수금 등록<br/>(Excel 수동 취합)"]
    B -->|등록| C["③ 세금계산서<br/>(위하고 이중 입력)"]
    C -->|발행| D["④ 기안/결재<br/>(위하고 전자결재)"]
    D -->|결재| E["⑤ 월결산<br/>(CS → 세무법인<br/>→ 위하고)"]
```

| 단계 | 담당 | 사용 시스템 | 수작업 여부 |
|------|------|-----------|:----------:|
| ① 입금 확인·배분 | 경영지원팀 | 신한 인사이트 뱅크 → Excel <br>→ 구글 드라이브 | **수동**<br>(핵심 병목) |
| ② 수금 등록 (투트랙) | 경영지원팀 / 각 담당자 | Excel + 나이스페이 | **수동** |
| ③ 세금계산서 | 경영지원팀 | 위하고(WEHAGO) (이중 입력) | **수동**<br>(이중 작업) |
| ④ 기안/결재 | 경영지원팀 | 위하고(WEHAGO) | 수동 |
| ⑤ 월결산 | 경영지원팀 + 세무법인 | CS 시스템 → Excel 서식 정리 <br>→ 세무법인 → 위하고(WEHAGO) | 수동 |
...
| 4. 마감/홍보 | 송고→발주→**디지털 업로드**→정산→북헌팅 | 편집장 + 디자이너 + 기자 | 인쇄소, **홈페이지 어드민**, 온라인서점 | **수동**<br>(업로드) |
...
> **[참고]** 인터뷰 완료 현황
>
> 5개 팀 인터뷰 + CEO 인터뷰 완료. 정기구독팀·경영지원팀 실측 포함, 편집팀·콜센터·영업추진팀 정성 확인.

```mermaid
graph TD
    root["[데이터]<br>일일 수작업 시간 추정<br/>(전 직원 합산)<br/><br/>CS/판매/정산 3개 팀: 7~10시간/일<br/>편집팀: 월 30~50시간 (월별 변동)<br/>콜센터: 이중 기록 + CTI 미연동 추가 소요<br/>영업추진팀: 이중·삼중 입력 + 엑셀 의존<br/>월간 합산: 170~250시간/월 + α"]
    
    team1["[정기구독팀]<br>인터뷰 확인<br/>정황규, 어은진<br/>약 4~5시간/일<br/>────<br/>외부몰 주문 수집+Excel 표준화<br/>+CS 등록+검수: 3~4시간 (일괄 배치)<br/>취소/환불(나이스페이 다중 계정): 수시<br/>선발송 처리: 수시<br/>문화누리카드 수기 결제: 수시<br/>재고 보정: 월 1회<br/>개인정보 수동 파기: 월 1회<br/>일반계좌 입금 매칭: 수시"]
    
    team2["[영업추진팀]<br>인터뷰 확인<br/>이성수, 권지은<br/>이중·삼중 입력 + 엑셀 전적 의존<br/>────<br/>고려출판물류→위하고 이중 입력: 매일<br/>입금 내역→관리 엑셀→위하고 삼중 입력: 수시<br/>통장 거래 내역 수작업 대조: 수시<br/>다량 특판 개인 엑셀 관리: 매일<br/>총판 출고 부수 협의(전화)→엑셀: 매월<br/>반품 순매출→위하고 등록: 매월<br/>편집팀 인쇄 완료 시점 수동 확인: 수시<br/>카드 매출전표 팩스/메일 발송: 수시"]
    
    team3["[경영지원팀]<br>인터뷰 확인<br/>송윤경, 김나현<br/>약 1.5~2시간/일<br/>────<br/>입금 내역 조회→배분: 매일 아침<br/>CS→위하고 양식 엑셀 재가공: 수시<br/>선수수익 현황표→세무법인: 월 1회<br/>인세 계산 수기 작성: 수시<br/>투트랙(CS↔위하고) 이중 관리"]
    
    team4["[편집팀]<br>인터뷰 확인<br/>이민애 외 8명<br/>월 30~50시간 (비정기)<br/>────<br/>디지털 업로드+리사이즈: 60~120분/월<br/>원고료 정산(엑셀→WEHAGO): 30~60분/월<br/>순환 교정(종이 기반): 연인원 20~30시간/월<br/>공모전 원고 배분·검토: 연 3회 집중<br/>어드민→CMS 복사·붙여넣기: 수시<br/>오프라인 원고 스캔: 수시"]
    
    team5["[외주콜센터]<br>인터뷰 확인<br/>더아이앤오 4명 (OB2+IB1+관리1)<br/>이중 기록 + CTI 미연동 추가 소요<br/>근무 10시~17시<br/>IB 일평균 75~80건 (월 1,500건)<br/>OB 성공 일 150~200건 (트라이 300~500건)<br/>상담 1건 30초~2분30초<br/>────<br/>CTI 미연동 전화번호 수동 조회: 건당<br/>CS 시스템+엑셀 이중 기록: 상담 건마다<br/>오토콜 양식 정제: 데이터 수령 시<br/>메모 개별 업로드: 건당 반복<br/>데일리 마감 리포트: 매일<br/>마케팅 동의 수기 기재: 건마다"]
...
| 화면 시연 인터뷰 일정 확정 | 완료 (인터뷰와 병행 수행) |
| CS 담당자 인터뷰 완료 | 완료 (정기구독팀, 2026-03-17) |
| 운영 담당자 인터뷰 완료 | 완료 (영업추진팀 2026-04-02, <br>경영지원팀 2026-03-19) |
...
> - 종이 기반 유지 확정. 디지털 전환 시도했으나 실무자들의 업무 편의상 현행 유지 (편집팀 인터뷰 VD3). 인터뷰 병행 관찰로 확인 완료
