...
| 인터뷰 | 실무자 5명 + 외부 개발자 1명 대상, Pain Point 식별 | 🔄 진행중 (정기구독팀 ✅) |
...
> 🔶 = 추정 (인터뷰/워크 쉐도잉 미실시). ✅ = 정기구독팀 인터뷰(2026-03-17) 확인 완료.
>
> ⚠️ **수작업 #4 (CMS 권한 수동 부여)**: 정기구독팀 인터뷰 결과, 해당 팀 소관 업무가 아님을 확인. 편집팀 인터뷰에서 교차 검증 필요.
...
> 🔶 인터뷰/워크 쉐도잉 전 문서 기반 추정치. ✅ 정기구독팀은 인터뷰(2026-03-17) 확인 완료.

```mermaid
graph TD
    root["📊 일일 수작업 시간 추정<br/>(전 직원 합산)<br/><br/>CS/판매/정산 3개 팀: 7~10시간/일<br/>편집팀: 월 30~50시간 (월별 변동)<br/>월간 합산: 170~250시간/월"]
...
| **외부 채널 수집** | #1, #2, #5, #6, #24 | 5건 | ⭐⭐⭐ (API 직접 연동 시) |
| **수동 검증/대조** | #5, #6, #11, #21, #22 | 5건 | ⭐⭐ (자동 검증 + 예외만 수동) |
| **결제/정산 처리** | #23, #26 | 2건 | ⭐⭐⭐ (결제 API 통합 시 완전 자동화) |
| **물류/출력** | #8, #18, #25 | 3건 | ⭐⭐ (배송 API 연동 + 선발송 로직 단순화) |
| **특수 업무 프로세스** | #19, #20, #27 | 3건 | ⭐⭐ (문화누리 온라인결제, 기부처 자동매칭, 회원 통폐합) |
...
1. **시간 낭비**
   - 일일 수작업 총 소요 시간: 7\~10시간 (CS/판매/정산 3개 팀, ✅ 정기구독팀 실측 포함) + 편집팀 별도
   - 월간 환산: 170\~250시간 (3개 팀) + 편집팀 🔶 30\~50시간 (디지털 업로드·정산·교정 등)
   - 전체 근무시간 대비 수작업 비율: 약 45\~60% (정기구독팀 ✅ 실측 기준 50%+)

2. **오류 발생**
   - 입력 오류 위험 항목: 27건 중 14건이 "상" 이상
...
> 기존 10건 + ✅ 인터뷰 확인 6건 = 총 16건. 신규 항목은 ⑪\~⑯으로 표기.

```mermaid
quadrantChart
    title 병목 영향도 매트릭스 (16건)
    x-axis 낮음 --> 높음 (빈도: 일일)
    y-axis 낮음 --> 높음 (영향도)
...
> 기존 10건 + ✅ 인터뷰 확인 6건 = 총 16건. 신규 항목은 ✅ 표시.
...
- [x] 결제 확인 → 권한 부여까지 실제 소요 시간? → ⚠️ CMS 권한 부여는 정기구독팀 소관이 아님 확인. 편집팀 인터뷰에서 재확인 필요
- [x] 수작업이 발생하는 단계는? → ✅ 27건 식별 완료 (초기 17건 + 인터뷰 10건 추가)
...
- [x] 권한 부여: 결제 확인 후 CMS에 별도 접속하여 수동 처리 — 실제 맞는지? → ⚠️ 정기구독팀 소관이 아님 확인. 편집팀에서 재확인 필요
