...
> **최종 산출물**: RFP (제안요청서)  
> **백로그 최종 수정일**: 2026-03-03 (Session 12)  
...
| `investigation-guide.md` | ~380 | 95% | ★★★★★ | **가장 실전적 문서**. 리버스 엔지니어링 전략, 리스크 대응, 인터뷰 전략, 체크리스트 매우 상세. 역공학 수행 결과(151t 분류, 56 로직, 6건 리스크), Phase 3 체크리스트 갱신 (수작업 12건/병목 7건 식별 결과 반영, 요구사항 48건). **VPN 접근 후 실물 검증 필요** |
| `system-diagnosis.md` | 301 | 90% | ★★★★★ | AS-IS 아키텍처 다이어그램, ERD, 151t 전수 목록, 49개 비즈니스 로직 상세, 수작업 아키텍처 매핑, Cross-DB 관계도, 도메인별 로직 분류, 성숙도 평가. **VPN 접근 후 실물 검증 필요** |
| `business-analysis.md` | ~350 | 85% | ★★★★ | 수작업 12건 식별 (유형별 분류: 시스템간 이관 4건, 외부 채널 4건, 수동 검증 3건, 물류 1건), 병목 7건 (영향도 매트릭스), 일일 수작업 6.5~9시간 추정, 자동화 75~80% 절감 기대. **워크 쉐도잉 미수행, 인터뷰 후 정량 확정 필요** |
| `interview.md` | 692 | 30% | ★★★ | 인터뷰 대상 10명 실명/역할 확정, 팀명 실제 조직 반영. **모든 결과 섹션 비어있음 (인터뷰 미수행)** |
| `requirements.md` | ~400 | 85% | ★★★★★ | 기능 요구사항 48건(12모듈), 비기능 11건 수치 확정 (동시접속 20명, 응답 3초, 가용성 99.5%), 우선순위 매트릭스 (Must 23/Should 13/Nice 12), 추적 매트릭스 신설. **인터뷰 후 우선순위 최종 검증 필요** |
...
| `data-integration.md` | 550 | 85% | ★★★★★ | 8개 도메인 23개 엔티티 TO-BE ERD, 13개 테이블 MSSQL 컬럼 정의, 요구사항 48건 교차 검증 매트릭스 (누락 0건), 데이터 동기화 방안 구체화 (외부몰 수집, CMS 자동 부여 30초, 구독 만료 스케줄러, ERP 연동). **인터뷰 후 검증 필요** |
| `process-automation.md` | 395 | 90% | ★★★★★ | 수작업 12건↔자동화 매핑 (건별 잔여 수작업률), 병목 7건↔우선순위↔요구사항 교차 매핑, 팀별 기대효과 (정기구독 75%, 영업추진 80%, 경영지원 85% 절감), 자동화 우선순위 8건, 구현 로드맵 Phase 1/2. **인터뷰 후 AS-IS 정량 수치 확정 필요** |
...
| `rfp-preparation.md` | 700 | 90% | ★★★★★ | **핵심 산출물 문서**. RFP 대규모 보강: 사업 배경 5대 문제점, 사업 범위(12모듈+이관+연동), 기능 48건/비기능 11건, 비즈니스 로직 56건 (홈페이지 7 SP 포함), §2.4 업무 프로세스 요구사항 신설 (수작업 12건, 병목 3건 Critical, BPR 우선순위 8건), **예산 FP 955~2,095 (2.5~5억)**, 평가 기준 상세화. **인터뷰 후 보정 6건 필요** |
...
2. **정량 데이터 일부 부재**: 워크 쉐도잉 미수행으로 수작업 시간/오류율은 추정치 상태
