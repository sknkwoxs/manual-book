...
| `business-overview.md` | ~160 | 60% | ★★★ | **기대 효과 정량 지표가 모두 `__%` 플레이스홀더**. 조직도, 사업배경 텍스트는 양호 |
| `as-is-to-be.md` | ~180 | 70% | ★★★★ | AS-IS/TO-BE 비교표 양호. **AS-IS 상세 수치(데이터량, 처리시간)가 비어있음** |
...
| `system-diagnosis.md` | 301 | 40% | ★★★ | AS-IS 아키텍처 ASCII 다이어그램 우수. **ERD 전체 섹션 플레이스홀더**, AS-IS 데이터 흐름 분석 미완, 문제점 식별 빈칸 |
| `business-analysis.md` | 166 | 20% | ★★ | 워크 쉐도잉 방법론은 잘 정리됨. **모든 정량 필드 `__`**, 수동 프로세스 목록/병목 분석 전부 비어있음 |
| `interview.md` | 692 | 15% | ★★★ | **분량 최대**. 3종 인터뷰 템플릿(실사용자, 외부개발자, 화면워크스루) 잘 구성. **모든 결과 섹션 완전히 비어있음** |
| `requirements.md` | 175 | 25% | ★★★ | FR/NFR 요구사항 ID 체계 잘 잡힘. **모든 "현행 지원" = "확인 필요"**, 우선순위 미결정 |
...
| `data-integration.md` | 222 | 50% | ★★★ | 개념 ERD(Customer, Subscription, Order, CsTicket 등) 초안 있음. **AS-IS 매핑 테이블 비어있음**, 테이블 정의 3개만 |
...
| `migration-plan.md` | 224 | 60% | ★★★★ | ETL 프로세스(Extract/Transform/Load) 코드 예시, 검증 방안, 롤백 계획 상세. **이관 대상 데이터량 모두 `__만 건`**, 테이블 매핑 비어있음 |
...
|------|:-------:|:----------:|-----------|
| Phase 1 (프로젝트 개요) | 5 | **77%** | 정량 지표 플레이스홀더 |
| Phase 2 (현행 분석) | 8 | **50%** | 인터뷰/분석 결과 전무, 정량 데이터 없음 |
| Phase 3 (목표 설계) | 7 | **72%** | AS-IS 데이터 미반영, 기술 선택 미확정 |
| Phase 4 (이행 계획) | 3 | **67%** | 예산/데이터량 미산정 |
| Phase 5 (미래 로드맵) | 4 | **70%** | 정량 효과 미측정 |
| 부록 | 3 | **90%** | - |
| **전체** | **30** | **67%** | **Phase 2 현행 분석이 병목** |
...
|:----:|------|:----------:|:----------:|------|
| 1 | interview.md | 15% | 80% | Week 4 |
| 2 | system-diagnosis.md | 40% | 80% | Week 4 |
| 3 | business-analysis.md | 20% | 80% | Week 4 |
| 4 | requirements.md | 25% | 90% | Week 5 |
| 5 | data-integration.md | 50% | 90% | Week 6 |
