...
| **NAS** | Network Attached Storage. 네트워크에 연결된 파일 공유 저장소 |
| **VPN** | Virtual Private Network. 가상 사설망. 외부에서 사내 네트워크에 안전하게 접속하는 기술 |
| **EOL** | End of Life. 소프트웨어/하드웨어의 공식 지원이 종료된 상태 |
| **WebRTC** | Web Real-Time Communication. 브라우저 간 플러그인 없이 실시간 음성/영상 통신을 가능하게 하는 기술 |
| **SIP** | Session Initiation Protocol. VoIP 통화의 연결·종료를 제어하는 통신 프로토콜 |
...
| **eGovFrame** | 전자정부 표준 프레임워크. 한국 공공기관/정부과제에 사용되는 Java 기반 개발 프레임워크 |
| **ActiveX** | Microsoft의 브라우저 플러그인 기술. Windows/IE 환경에서만 동작하는 레거시 기술 |
...
| **CMS** | Content Management System. 콘텐츠 관리 시스템. 좋은생각 월간지 원고 아카이브 시스템 (CS 통합 범위 외, 별도 운영) |
| **선수수익** | Deferred Revenue. 서비스 제공 전 선수령한 수익을 제공 시점에 매출로 인식(차감)하는 회계 처리 방식 |
| **문화누리카드** | 정부 발행 문화복지 카드. 기초생활수급자·차상위계층 대상 문화·여행·체육 지원 |
| **안심번호** | 외부몰(네이버/쿠팡 등) 구매자의 실제 전화번호 대신 제공되는 임시 연락처 |
| **총판** | 월간지를 일괄 구매하여 재판매하는 중간 유통업체 |
| **다량·특판 (B2B)** | 기업/기관 대상 대량 판매. 기부·복지·사내배포 등 목적의 일괄 구매 거래 |
| **새벽햇살** | 교정기관(교도소 등)에 월간지를 기부하는 사회공헌 프로그램 |
| **에스컬레이션** | 처리 불가한 문의를 상위 담당자 또는 본사로 이관하는 것 |
| **지로** | 금융결제원을 통한 납부 방식. 고지서 기반 계좌이체 결제 수단 |
| **구독** | 좋은생각 콘텐츠(월간지)를 정기적으로 이용하는 유료 서비스 |
...
| 용어 | 정의 |
|------|------|
| **CTI** | Computer Telephony Integration. 전화 인입 시 발신번호 기반 고객 정보를 자동 팝업 표시하는 시스템 |
| **오토콜** | 아웃바운드 자동 전화 발신 시스템. 대상 목록에 자동으로 전화를 걸어 상담원에게 연결 |
| **나이스페이 (NicePay)** | PG사. 신용카드/ARS/가상계좌/현금영수증 등 온라인 결제를 처리하는 서비스 |
| **위하고 (WEHAGO)** | 더존비즈온 클라우드 ERP/그룹웨어. 회계, 세금계산서, 전자결재, 메신저 등 경영지원 기능 제공 |
| **고려출판물류** | 출판 유통 전문 물류 대행사. 서점 배본 및 반품 처리 담당 |
| **인사이트 뱅크 (신한)** | 신한은행 기업 인터넷뱅킹 서비스. 다수 계좌의 입출금 내역을 통합 조회·다운로드 가능 |
| **SCM** | Supply Chain Management. 공급망 관리 시스템. 온라인서점 등에 상품을 공급·관리하는 시스템 |
...
| **화면 시연 인터뷰** | 인터뷰 중 실무자가 실제 시스템 화면을 시연하며 업무 절차와 데이터 흐름을 설명하도록 하여 업무 흐름을 파악하는 방법 |
