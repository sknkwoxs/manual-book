...
본 장은 (주)좋은생각의 현행 시스템·업무 현황과 본 사업에서 충족해야 할 비기능 요구사항(14건), 데이터 이관 요구사항, 외부 시스템 연동 요구사항을 정의함. 기능 요구사항 79건의 상세는 제4장 4.3.2 SRE에 기재함.
...
가. (주)좋은생각의 정기구독·콘텐츠 운영을 지원하는 현행 시스템은 다음과 같이 구성되어 있음.
...
| ERP (위하고) | 매출·정산·세금계산서 | 외부 SaaS | 현재 엑셀 양방향. API 연동 대상 |
...
자. **문서화 부재 (Medium)**: API 명세·DB 스키마 문서 부재.
...
| 매출 → ERP 입력 | API 미연동, 이중 입력 + 엑셀 재가공 | 매일 30~60분 + 데이터 불일치 위험 |
...
나. 기능 79건은 10개 도메인(대시보드 5, 고객 9, 구독 11, 주문 5, 배송 9, 결제·정산 16, CS 상담 10, 선물 4, 재고/도서 3, 시스템 7)으로 구성되며, 각 요구사항은 SRE-001 ~ SRE-079의 고유번호를 부여함.
...
| :--- | :--- | :--- |
| 자사 홈페이지 | REST API | 실시간 |
| CMS | REST API | 실시간 (권한 부여) |
| 외부몰 (네이버 스마트스토어, 쿠팡) | Playauto API 또는 직접 API | 5~10분 폴링 |
| ERP (위하고) | REST API | 일 1회 배치 + 필요 시 실시간 |
| PG (나이스페이) | REST API | 실시간 |
| CTI | API 또는 SDK | 실시간 |
| 금융결제원 (지로) | 파일 업로드 (현행 유지) | 일 1회 |
| 택배사 (CJ대한통운 등) | API 또는 파일 업로드 | 일 1~2회 |
| 외부 콜센터 (더아이앤오) | 웹 접속 권한 부여 | 실시간 |
...
다. **상세 명세**: 제4장 4.3.4 DAR(데이터 요구사항) 및 4.3.1 STR(시스템 요구사항) 참조
...
| 운영 | 문서화 | API 명세서, DB 스키마, 운영 매뉴얼, 사용자 매뉴얼 제출 |
