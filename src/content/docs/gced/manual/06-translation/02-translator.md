...
:::tip[내 작업이 많으면 My task]
사이드바 **Translation** 메뉴 → **My task** (`/manage-translate/my_task`)는
로그인한 나에게 할당된 작업만 보여줍니다. Pending 탭과 함께 즐겨찾기해 두면
확인할 때 가장 빠릅니다.
:::
...
:::note[할당되면 이메일이 옵니다]
작업이 할당되면 `gcedch@unescoapceiu.org`에서
**"번역 작업이 할당되었습니다: {제목}"** 메일이 옵니다
(External 표시가 붙을 수 있으니 스팸함도 확인하세요).
메일 본문의 **번역 작업 열기** 버튼으로 바로 이동할 수 있습니다.
메일이 안 왔다면 아직 할당되지 않은 상태일 수 있으니
Eligible 탭을 확인한 뒤 관리자에게 할당 완료 여부를 물어보세요.
:::
...
3. **번역 진행**: 번역 작업 중
4. **In review**: 검토 대기 (DB 총괄관리자 확인 필요 — 검토 절차는 [관리자용 번역 검토 화면](./01-admin#번역-검토-화면) 참고)
5. **Closed**: 최종 완료
